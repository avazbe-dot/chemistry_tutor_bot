...
# Публичный адрес сервиса — для само-пинга, чтобы бесплатный тариф Render не
# усыплял процесс из-за отсутствия входящего HTTP-трафика (без него хостинг
# останавливает процесс примерно через 15 минут бездействия). На Render эта
# переменная (RENDER_EXTERNAL_URL) выставляется автоматически.
SELF_PING_URL = (
    os.environ.get("RENDER_EXTERNAL_URL", "").strip()
    or os.environ.get("SELF_PING_URL", "").strip()
)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/backup — получить файлом резервную копию всех материалов (только админ)"
    )
...
def _send_backup_to_admin_sync():
    """Отправляет ADMIN_ID файл с текущим содержимым STORE через Telegram Bot
    API напрямую (без async), чтобы можно было вызывать из фонового потока —
    как для ежедневной автовыгрузки, так и как общий помощник."""
    try:
        data = json.dumps(STORE, ensure_ascii=False, indent=2).encode("utf-8")
        filename = f"backup_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.json"
        caption = f"📦 Автоматическая резервная копия на {datetime.now().strftime('%d.%m.%Y %H:%M')} ({len(STORE)} ключей)."
        ...
        with urllib.request.urlopen(req, timeout=20):
            pass
        print("✅ Ежедневная резервная копия отправлена админу.")
    except Exception as e:
        print(f"⚠ Не удалось отправить автоматическую резервную копию: {e}")


def _run_daily_backup_loop():
    """Раз в сутки присылает ADMIN_ID резервную копию базы материалов."""
    while True:
        threading.Event().wait(24 * 60 * 60)  # 24 часа
        _send_backup_to_admin_sync()


def _run_keep_alive_loop():
    """Раз в 10 минут дёргает собственный публичный URL, чтобы бесплатный
    тариф Render не усыпил процесс из-за отсутствия входящего трафика.
    Если SELF_PING_URL/RENDER_EXTERNAL_URL не задан (например, при
    локальном запуске в Pydroid) — просто ничего не делает."""
    if not SELF_PING_URL:
        return
    while True:
        try:
            req = urllib.request.Request(
                SELF_PING_URL, headers={"User-Agent": "Mozilla/5.0 (keep-alive)"}
            )
            urllib.request.urlopen(req, timeout=10).read()
        except Exception as e:
            print(f"⚠ Не удалось выполнить keep-alive пинг: {e}")
        threading.Event().wait(600)  # 10 минут


async def backup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Присылает админу текущее содержимое базы (STORE) файлом — резервная
    копия на случай, если материалы вдруг снова пропадут из облака."""
    user_id = update.effective_user.id
    if user_id != ADMIN_ID:
    ...
    await update.message.reply_document(
        ...
        filename=filename,
        caption=f"📦 Резервная копия материалов на {datetime.now().strftime('%d.%m.%Y %H:%M')} ({len(STORE)} ключей).",
    )
...
def main():
    ...
    threading.Thread(target=_run_keep_alive_loop, daemon=True).start()
    threading.Thread(target=_run_daily_backup_loop, daemon=True).start()
    if SELF_PING_URL:
        print(f"🔁 Само-пинг включён: раз в 10 минут буду обращаться к {SELF_PING_URL}")
    else:
